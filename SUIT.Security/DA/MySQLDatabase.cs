...
namespace SUIT.Security.DA
{
    ...
    public class MySQLDatabase : IDisposable
    {
        ...
        public MySQLDatabase(string connectionString)
        {
            ...
            //_connection = new MySqlConnection(new IStartup().Configuration["ConnectionStrings:DefaultConnection"]);
        ...
        /// <param name="parameters">Optional parameters to pass to the query</param>
        /// <returns>The count of records affected by the MySQL statement</returns>
        public int ExecuteSP(string spName, Dictionary<string, object> parameters)
        {
            int result = 0;

            if (String.IsNullOrEmpty(spName))
            {
                throw new ArgumentException("Stored procedure is missing");
            }
            ...
            try
            {
                ...
                MySqlCommand cmd = new MySqlCommand(spName, _connection);
        ...
        public object ExecuteSPGetId(string spName, Dictionary<string, object> parameters)
        {
            object result = null;
        ...
        /// <returns>A list of a Dictionary of Key, values pairs representing the 
        /// ColumnName and corresponding value</returns>
        public List<Dictionary<string, string>> QuerySP(string spName, Dictionary<string, object> parameters)
        {
            List<Dictionary<string, string>> rows = null;
            if (String.IsNullOrEmpty(spName))
            {
            ...
            {
                EnsureConnectionOpen();
                //var command = CreateCommand(spName, parameters);
